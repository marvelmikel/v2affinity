    ...
    public function assets(Request $request)
    {
        try {
            if (class_exists(\League\Flysystem\Util::class)) {
                // Flysystem 1.x
                $path = dirname(__DIR__, 2).'/publishable/assets/'.\League\Flysystem\Util::normalizeRelativePath(urldecode($request->path));
                ...
                $path = dirname(__DIR__, 2).'/publishable/assets/'. $normalizer->normalizePath(urldecode($request->path));
            }
