<div x-data="{ load: @entangle('load').defer }" class="w-full relative min-h-screen flex flex-col lg:flex-row">
        ...
        <div class="bg-slate-800 lg:rounded-lg h-full flex flex-col justify-between p-6">
            ...
            <ul class="flex lg:flex-col justify-center lg:justify-start gap-6">
                <li class="flex gap-4 items-center">
                    ...
                    <strong
                        class="hidden sm:block font-semibold @if($step > 1) text-green-600 @elseif($step === 1) text-slate-400 @else text-slate-400 @endif">Account<span
                            class="hidden lg:inline"> Details</span></strong>
                </li>
                <li class="flex gap-4 items-center">
                    <span class="
                        @if($step > 2)
                    ...
                    <strong
                        class="hidden sm:block font-semibold @if($step > 2) text-green-600 @elseif($step === 2) text-slate-400 @else text-slate-400 @endif">Company<span
                            class="hidden lg:inline"> Details</span></strong>
                </li>
                <li class="flex gap-4 items-center">
                    <span class="
                    ...
                    <strong
                        class="hidden sm:block font-semibold @if($step > 3) text-green-600 @elseif($step === 3) text-slate-400 @else text-slate-400 @endif">Subscription</strong>
                </li>
                <li class="flex gap-4 items-center">
                    <span class="
                        @if($step > 4)
                    ...
                    <strong
                        class="hidden sm:block font-semibold @if($step > 4) text-green-600 @elseif($step === 4) text-slate-400 @else text-slate-400 @endif">Terms<span
                            class="hidden lg:inline"> & Conditions</span></strong>
                </li>
                <li class="flex gap-4 items-center">
                    <span class="
                        @if($step > 5)
                    ...
                    <strong
                        class="hidden sm:block font-semibold @if($step > 5) text-green-600 @elseif($step === 5) text-slate-400 @else text-slate-400 @endif">Billing<span
                            class="hidden lg:inline"> Details</span></strong>
                </li>
                <li class="flex gap-4 items-center">
                    <span class="
                    ...
                    <strong
                        class="hidden sm:block font-semibold @if($step > 6) text-green-600 @elseif($step === 6) text-slate-400 @else text-slate-400 @endif">Payment</strong>
                </li>
            </ul>
            <div class="hidden lg:block">
                ...
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <x-button-link :href="route('logout')" format="wire" type="submit"
                        class="w-full border-slate-300 text-slate-300 text-center hover:text-main-color hover:border-main-color"
                        onclick="event.preventDefault(); this.closest('form').submit();">
                        {{ __('Log Out') }}
                    </x-button-link>
                </form>
                @else
                <x-button-link href="{{ route('voyager.login') }}" format="wire" type="button"
                    class="w-full border-slate-300 text-slate-300 text-center hover:text-main-color hover:border-main-color">Already
                    have an account?</x-button-link>
                @endif
            </div>
        </div>
    </aside>
        ...
        @switch($step)
            ...
            <form wire:key="step_1" @submit.prevent="load = true, $wire.register_user()" class="space-y-6">
                ...
                <div class="grid grid-cols-2 gap-6">
                    ...
                    <p class="col-span-2">
                        ...
                        <x-input wire:model="user.password_confirmation" name="password_confirmation"
            ...
            <form wire:key="step_3" @submit.prevent="load = true, $wire.register_company()" class="flex flex-col gap-6">
                <div>
                    ...
                    <p class="font-medium lg:text-lg text-slate-500">Please enter your Company details. These details can be
                        changed by admins after completion.</p>
                </div>
                        ...
                        <fieldset>
                            <div class="grid grid-cols-2 gap-6">
                                <p class="col-span-1">
                                    ...
                                    <x-input wire:model="company.company_name" name="company_name" id="company_name"
                                        type="text" required />
                                    <x-input-error :messages="$errors->get('company.company_name')" class="mt-2" />
                                </p>
                                <p class="col-span-1">
                                    <x-label for="company_phone">Phone No.</x-label>
                                    <x-input wire:model="company.company_phone" name="company_phone" id="company_phone"
                                        type="tel" />
                                ...
                                <p class="col-span-1">
                                    ...
                                    <x-input wire:model="company.company_email" name="company_email" id="company_email"
                                        type="text" />
                                    <x-input-error :messages="$errors->get('company.company_email')" class="mt-2" />
                                </p>
                                <p class="col-span-1">
                                    <x-label for="company_number">Company Reg<span class="hidden lg:inline">istration</span>
                                        No.</x-label>
                                    <x-input wire:model="company.company_number" name="company_number" id="company_number"
                                        type="text" />
                                    <x-input-error :messages="$errors->get('company.company_number')" class="mt-2" />
                                </p>
                                <p class="col-span-1">
                                    <x-label for="vat_number">VAT Reg<span class="hidden lg:inline">istration</span>
                                        No.</x-label>
                                    <x-input wire:model="company.vat_number" name="vat_number" id="vat_number"
                                        type="text" />
                                    <x-input-error :messages="$errors->get('company.vat_number')" class="mt-2" />
                                </p>
                                <p class="col-span-1">
                                    <x-label for="company_address">Full Address</x-label>
                                    <x-input wire:model="company.company_address" name="company_address"
                        ...
                        <label wire:click="activateFreeTrial()" style="background-color: #e5e7eb;"
                            ...
                            <div class="flex items-center justify-between">
                                <p class="leading-none">
                                    ...
                                    <span class="font-medium text-slate-400 text-sm">No credit card required</span>
                                </p>
                                <i
                                    class="fa-circle-check fa-solid group-peer-checked:opacity-100 opacity-0 text-2xl text-green-500"></i>
                            </div>
    ...
    <style>
        ...
        .copy-icon {
            cursor: pointer;
            color: #3498db;
            /* Use the color that suits your design */
            margin-left: 10px;
        }
    </style>
